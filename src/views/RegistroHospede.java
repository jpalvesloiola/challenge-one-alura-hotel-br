...
public class RegistroHospede extends JFrame {
	...
	public void salvarHospede() {
		...
		hospedeController.salvar(txtNome.getText(), txtSobrenome.getText(), converteData(txtDataN), txtNacionalidade.getSelectedItem().toString(), txtTelefone.getText(), Nreserva);
		JOptionPane.showMessageDialog(contentPane, "Registro Salvo, Número de Reserva: " + txtNreserva.getText());
	}
	

	
	private Date converteData(JDateChooser txtData) {
		return Date.valueOf(((JTextField)txtData.getDateEditor().getUiComponent()).getText()) ;
	}
	
	public void setTxtNreserva(int id) {
		this.txtNreserva.setText("" + id);
	}
	
	public String getTxtNreserva() {
		return txtNreserva.getText();
	}
}
